...
function TspImp (map) {
	...
	function addWaypointWithLabel(latLng, label, ico) {//....
		GoogleProvider.tsp.addWaypointWithLabel(latLng, label, addWaypointSuccessCallbackZoom);
		this.ico = ico;
	}
	...
	function drawMarkers(updateViewport) {//.............
		removeOldMarkers();
		var waypoints = GoogleProvider.tsp.getWaypoints();
		var addresses = GoogleProvider.tsp.getAddresses();
		var labels = GoogleProvider.tsp.getLabels();
		for (var i = 0; i < waypoints.length; ++i) {
	...
	function directions(m, walking, avoid) {
		mode = m;
		GoogleProvider.tsp.setAvoidHighways(avoid);
		if (walking)
			GoogleProvider.tsp.setTravelMode(google.maps.DirectionsTravelMode.WALKING);
		else
			GoogleProvider.tsp.setTravelMode(google.maps.DirectionsTravelMode.DRIVING);
		if (m == 0)
			GoogleProvider.tsp.solveRoundTrip(onSolveCallback);
		else
			GoogleProvider.tsp.solveAtoZ(onSolveCallback);
	}
	...
	function onSolveCallback(myTsp) {
		var dirRes = GoogleProvider.tsp.getGDirections();
	...
	this.addWaypointWithLabel = addWaypointWithLabel;
